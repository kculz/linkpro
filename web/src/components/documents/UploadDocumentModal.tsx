    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-background/80 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="bg-surface border border-white/[0.05] rounded-[3rem] w-full max-w-xl shadow-[0_0_50px_rgba(0,0,0,0.5)] overflow-hidden animate-in zoom-in-95 duration-300 relative">
        ...
        <div className="flex items-center justify-between p-10 border-b border-white/[0.03] relative z-10">
          <div>
            <h2 className="text-3xl font-black text-white italic uppercase tracking-tighter decoration-primary decoration-4 underline underline-offset-8">Deposit Artifact</h2>
            <p className="text-white/20 text-[10px] font-bold mt-4 uppercase tracking-[0.2em] italic">Archive secure documentation in vault</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8 relative z-10">
          <div className="group relative border-2 border-dashed border-white/5 hover:border-primary/30 rounded-[2.5rem] p-12 text-center transition-all cursor-pointer bg-white/[0.01]">
            ...
            <p className="text-[10px] font-black text-white/20 uppercase tracking-widest italic group-hover:text-white transition-colors">Select target file to archive...</p>
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="col-span-2">
              <label className="block text-[10px] font-black text-white/20 mb-3 uppercase tracking-widest italic flex items-center gap-2">
                <FileText className="w-4 h-4 text-primary" /> Artifact Designation
              </label>
              <input
                required
                type="text"
            ...
            <div>
              <label className="block text-[10px] font-black text-white/20 mb-3 uppercase tracking-widest italic flex items-center gap-2">
                <ShieldCheck className="w-4 h-4 text-primary" /> Security Classification
              </label>
              <select
              ...
              >
                ...
                <option value="RECEIPT" className="bg-background">Financial Receipt</option>
                <option value="BLUEPRINT" className="bg-background">Technical Blueprint</option>
                <option value="CONTRACT" className="bg-background">Legal Contract</option>
                <option value="ID" className="bg-background">Identity Document</option>
                <option value="OTHER" className="bg-background">Other Asset</option>
              </select>
            </div>

            <div>
              <label className="block text-[10px] font-black text-white/20 mb-3 uppercase tracking-widest italic flex items-center gap-2">
                <Target className="w-4 h-4 text-primary" /> Relational Linkage
              </label>
              <select
                className="w-full bg-white/[0.02] px-6 py-4 rounded-2xl border border-white/[0.05] focus:border-primary/50 focus:ring-4 focus:ring-primary/5 outline-none transition-all text-white font-bold italic text-sm appearance-none"
              ...
              >
                <option value="GENERAL" className="bg-background">General Vault</option>
                <option value="PROPERTY" className="bg-background">Portfolio Asset</option>
                <option value="PROJECT" className="bg-background">Operational Project</option>
              </select>
            </div>
          ...
          <div className="pt-10 flex gap-6">
            ...
            >
              {loading ? <Loader2 className="w-6 h-6 animate-spin" /> : 'Confirm Deposit'}
            </button>
          </div>
        </form>
      </div>
